...
@Service(value = "Impl") //@Service : stereo type, Spring Framework 에게 컨포넌트임을 알려줌
public class MemoServiceImpl implements MemoService{

    MemoRepository memoRepository;
    public MemoServiceImpl(MemoRepository memoRepository) { // 생성자 주입
        //Spring Framework 가 MemoRepository 인터페이스를 구현한 인스턴스를 배정함
    ...
    public List<Memo> initialize() {
        List<Memo> result = new ArrayList<Memo>(); //Memo 객체를 원소로 갖는 리스트형 객체를 생성, 배정
        //테이블 초기화 코드
        IntStream.rangeClosed(1, 10).forEach(i -> {
            MemoEntity memo = MemoEntity.builder().memoText("hojun : " + i).build();
